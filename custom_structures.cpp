...
private:
    ...
    int hash(const string& key) const { return hash(key, capacity); }
...
public:
    HashTable(int cap = 16) : capacity(cap), liveCount(0) {
        buckets = new List<Entry<T>>[capacity];
    }

    HashTable(const HashTable& other)
        : capacity(other.capacity), liveCount(other.liveCount) {
        buckets = new List<Entry<T>>[capacity];
        for (int i = 0; i < capacity; i++) buckets[i] = other.buckets[i];
    }
    ...
    HashTable(HashTable&& other) noexcept
        : buckets(other.buckets), capacity(other.capacity), liveCount(other.liveCount) {
        other.buckets = nullptr; other.capacity = 0; other.liveCount = 0;
    }
    ...
    void insert(const string& key, const T& value) {
        if ((double)(liveCount + 1) / capacity > 0.75) rehash();
        int idx = hash(key);
    ...
    T* get(const string& key) {
        int idx = hash(key);
        for (int i = 0; i < buckets[idx].getSize(); i++)
            if (buckets[idx][i].key == key) return &buckets[idx][i].value;
        return nullptr;
    }
    ...
    // Duyệt tất cả entries, gọi callback(key, value)
    template<typename Func>
    void forEach(Func callback) const {
        for (int i = 0; i < capacity; i++)
            for (int j = 0; j < buckets[i].getSize(); j++)
                callback(buckets[i][j].key, buckets[i][j].value);
    }
...
public:
    ...
    T top() const {
        if (empty()) throw runtime_error("PriorityQueue rong!");
        return data[0];
    }
